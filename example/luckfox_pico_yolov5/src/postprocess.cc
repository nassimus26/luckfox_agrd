...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "/root/ai/model/coco_80_labels_list.txt"
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (app_ctx->is_quant) {
            validCount += process_i8_rv1106((int8_t *)(_outputs[i]->virt_addr), 
                            (int *)anchor[i], grid_h, grid_w, model_in_h, model_in_w,
                             stride, filterBoxes, objProbs,
